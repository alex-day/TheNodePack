...
public class PlayerManager
{
    ...
    public void addPlayer(EntityPlayerMP entityplayermp)
    {
        ...
        mcServer.configManager.sendPacketToPlayer(entityplayermp.username,new Packet3Chat("\247d[Server] Welcome the the Node Pack server v1.0"));
		mcServer.configManager.sendPacketToPlayer(entityplayermp.username,new Packet3Chat("\247d[Server] Behave or Bebooted."));
    ...
    private boolean func_544_a(int i, int j, int k, int l)
    {
        int i1 = i - k;
        int j1 = j - l;
        if(i1 < -field_28109_f || i1 > field_28109_f)
        {
            return false;
        }
        return j1 >= -field_28109_f && j1 <= field_28109_f;
    }
