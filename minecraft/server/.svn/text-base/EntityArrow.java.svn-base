...
public class EntityArrow extends Entity
{
    ...
    public void onUpdate()
    {
        ...
        if(i > 0)
        {
            ...
            if(axisalignedbb != null && axisalignedbb.func_28183_a(Vec3D.createVector(posX, posY, posZ)))
            {
                inGround = true;
            }
        }
        ...
        if(movingobjectposition != null)
        {
            if(movingobjectposition.entityHit != null)
            {
            	if(owner instanceof EntityPlayer) {
            		EntityPlayer ownerEP = (EntityPlayer)owner;
            		if(movingobjectposition.entityHit instanceof EntityPlayer) {
            			ownerEP.addNodeScore(-100*4);
            		} else {
            			if(movingobjectposition.entityHit instanceof EntityAnimal) {
            				ownerEP.addNodeScore(2*4);
            			} else if(movingobjectposition.entityHit instanceof EntityMob) {
            				ownerEP.addNodeScore(25*4);
            			} else if(movingobjectposition.entityHit instanceof EntityFlying) {
            				ownerEP.addNodeScore(40*4);
            			} else {
            				ownerEP.addNodeScore(1*4);
            			}
            		}
            	}
